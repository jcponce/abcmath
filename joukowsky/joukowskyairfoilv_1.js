...
let bx = 5.5; //Limit x to add particles
let by = 3.5; //limit y to add particles
...
function draw() {
    ...
    if (starting==false) {
        ...
        stroke(255);
        textSize(23);
        text("Touch on screen to start", 6*width/16, height/4);
    }
    ...
    textSize(1);
    fill(255);
    text('U='+sliderU.value(), -16, 11);
    text('C='+sliderC.value(), -8, 11);
    text('T='+sliderT.value(), 1, 11);
    fill(255);
    stroke(255);
    text('PLAY', 12, 11.2);
...
function controls() {
    ...
    //buttonTrace.position(800, 630);
    //buttonTrace.mousePressed(traceShow);
    //buttonTrace.style("font-size", "25px");
    
    sliderU = createSlider(0.1, 0.7, 0.3, 0.1);
    sliderU.position(65, 678);
    sliderU.style('width', '160px');
    
    sliderC = createSlider(-10, 10, 0, 0.1);
    sliderC.position(290, 678);
    sliderC.style('width', '160px');
    
    sliderT = createSlider(0, 1, 0, 0.01);
    sliderT.position(530, 678);
    sliderT.style('width', '160px');
    
    checkbox = createCheckbox('.', false); // create a checkbox with a name and an initial value
    checkbox.position(850, 655); // where it's located
    checkbox.changed(traceShow); // what happens when it is checed
